...
public class DVDCollectionAppView1 extends Pane implements DVDView{
    ...
    public void update(DVDCollection model, int selectedDVD) {
        ...
        ObservableList<Integer> lengths = FXCollections.observableArrayList();

        for(DVD i: model.getDVDList()) {
            titles.add(i.getTitle());
            dates.add(i.getYear());
            lengths.add(i.getDuration());
        }

        tList.setItems(titles);
        yList.setItems(dates);
        lList.setItems(lengths);
